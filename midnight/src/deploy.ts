/**
 * Deploy midnight contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { deriveOwnerSecret } from './commitments';

// @ts-expect-error Required for wallet sync
globalThis.WebSocket = WebSocket;

// Private circuit values are explicit arguments supplied by the local bridge;
// the generated contract itself has no Compact witness callbacks.
...
const OWNER_SECRET = deriveOwnerSecret(SEED);
{
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'private-docs');
...
const PrivateDocs = await import(pathToFileURL(contractPath).href);

const compiledContract = CompiledContract.make('private-docs', PrivateDocs.Contract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  const configuredPrivateStatePassword = process.env.PRIVATE_STATE_PASSWORD?.trim();
  if (configuredPrivateStatePassword && configuredPrivateStatePassword.length < 16) {
    throw new Error('PRIVATE_STATE_PASSWORD must contain at least 16 characters');
  }
  if (!configuredPrivateStatePassword && network !== 'undeployed') {
    throw new Error(
      'PRIVATE_STATE_PASSWORD is required for preview and preprod and must contain at least 16 characters',
    );
  }
  const privateStatePassword =
    configuredPrivateStatePassword || 'Local-Devnet-Development-Placeholder-1';
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'private-docs-contract-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy midnight to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // The owner secret is a private Compact constructor argument derived
      // from the wallet seed with a PrivateDocs-specific domain. The ledger
      // stores only its persistent hash commitment.
